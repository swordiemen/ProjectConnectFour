...
public class Board {
	...
	/**
	 * Creates a new ROWS*COLUMNS(from Constants.java) board, whose fields are
	 * all empty.
	 */
	public Board() {
	 ...
	 * 
	 * @param index
	 *            The field that has to be set.
	 * @param m
	 *            The mark the field has to be set.
	 */
	 ...
	 * 
	 * @param r
	 *            The row the mark has to be placed on.
	 * @param c
	 *            The column the mark has to be placed on.
	 * @param m
	 *            The mark to be set.
	 */
	...
	/**
	 * Adds a certain disc to a row. Returns true if a disc was added, false
	 * otherwise.
	 * 
	 * @param col
	 *            The col the mark has to be added to.
	 * @param m
	 *            The Mark of the inserted disc.
	...
	/**
	 * Gets the next empty field where a disc would be in, in a specified
	 * column.
	 * 
	 * @param c
	 *            The column to check.
	 * @return The index of the first (from below) empty field. -1 if there is
	 *         none.
	 */
	...
	/**
	 * Returns the Mark of the winner of the game. Requires hasWinner() to be
	 * true.
	 * 
	...
	public boolean hasDiagonal(Mark m) {
		// TODO optimalize by using WIN_DISCS to limit the amount of checking
		// that has to be done.
		// when you need 4 discs to win (default), there's no need to check
		// the first 3 beginning rows if there's an ascending diagonal.
		for (int c = 0; c <= col - Constants.WIN_DISCS; c++) { // checks the top
																// and bottom
																// row for
																// diagonals.
		...
		for (int r = 0; r < row; r++) { // checks the first column for
										// diagonals. Together with ^, this
	...
	/**
	 * Checks if a Mark m has 4 in a row on a a diagonal which goes downwards
	 * from point r, c.
	 * 
	 * @param r
	 *            The row to start checking on.
	 * @param c
	 *            The column to start checking on.
	 * @param m
	 *            The Mark to be checked.
	...
	public boolean hasDiagonalDescending(int r, int c, Mark m) {
		/*
		 * the following cases shouldn't happen, but will simply return false if
		 * it somehow still happens.decided not to throw a
		 * IndexOutOfBoundsException, as the following will most likely only
		 * occurwhen dealing with a off-by-one error, so the program shouldn't
		 * have any trouble continuing after this point.
		 */
		if (r < 0 || r >= row) {
	...
	/**
	 * Checks if a Mark m has 4 in a row on a a diagonal which goes upwards from
	 * point r, c.
	 * 
	 * @param r
	 *            The row to start checking on.
	 ...
	 * 
	 * @param i
	 *            The field where you want the info from.
	...
	/**
	 * Gets the winning fields of a game. Method should not be used if there is
	 * no winner.
	 * 
	...
	/**
	 * Checks if a specified column is full. i.e. whether getNextEntryInColumn
	 * returns -1.
	 * 
	 * @param c
	 *            The column to check.
