...
public class Lobby extends JFrame implements ActionListener, Runnable {
	...
	// public void run() {
	// try {
	// //Client c = new Client(InetAddress.getByName("localhost"), 1338,
	// "Sjon");
	...
	/**
	 * This method is called when the client has received a challenge. This will
	 * prompt a <code>JOptionPane</code>, where the client can choose to accept
	 * or decline the challenge. If the client accepts, it will send a
	 * <code>challangeAccept</code> with its name, otherwise it will send a
	 * <code>challengeRefused</code> with its name.
	 * 
	 * @param name
	...
	public void challenged(String name) {
		...
		int accepted = JOptionPane.showOptionDialog(this,
				  "Accept the challenge from " + name + "?", "", JOptionPane.OK_OPTION,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (showLeaderBoardButton.equals(source)) {
			...
			chatboxPane.setText(chatboxPane.getText()
					  + "[Server] Searching for someone for you to play with...\n");
			client.makeGame();
			...
			if (playerListList.getSelectedValue() != null) {
				chatboxPane.setText(chatboxPane.getText() + "[Server] You challenged "
						  + playerListList.getSelectedValue() + ".\n");
				client.sendChallenge(playerListList.getSelectedValue());
			} else {
				chatboxPane.setText(chatboxPane.getText()
						  + "[Server] You must select a player to challenge.\n");
			}
		} else if (sendButton.equals(source)) {
			// if(whisperBox.getSelectedIndex() == 0){
			// byte b = (byte) 0x101;
			// System.out.println("1e: " + b % 2 + " 2e: " + (b >> 1 % 2) +
			// " 3e: " + (b >> 2 % 2));
			// //client.sendTell(chatField.getText());
			// }else{
			//
			// //chatboxPane.setText(chatboxPane.getText() + "[Chat] Je target "
			// + whisperBox.getSelectedItem() + ".\n");
			// //client.sendWhisper(chatField.getText(),
			// whisperBox.getSelectedItem());
			// }
	...
	/**
	 * When a <code>Client</code> receives an updated player list from the
	 * <code>Server</code>, this method is called. Updates the
	 * <code>playerList</code> of this <code>Lobby</code>.
	 * 
	 * @param argPlayerList
	 *            The updated <code>playerList</code>
	 */
	...
	/**
	 * This method is called when the <code>Client</code> receives a chat
	 * message from the <code>Server</code>. It will then be displayed in this
	 * <code>Lobby</code>'s <code>chatboxPane</code>.
	 * 
	 * @param sender
	 *            The sender of the chat message.
	 * @param msg
	 *            The chat message of the sender.
	 */
